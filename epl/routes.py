...
@app.route('/')
def index():
  return render_template('index.html',title='EPL Home Page')
...
def info_club(id):
  ...
  return render_template('clubs/info_club.html',title='Club Info Page',club=club)
...
def new_player():
  ...
  if request.method == 'POST':
    ...
    player = Player(name=name,position=position,nationality=nationality,goal=goal,squad_no=squad_no,img=img,club_id=club_id)
